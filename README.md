...
1. **Launch Instance**: Spin up an EC2 instance using the **Ubuntu 22.04 LTS** AMI on a free-tier `t2.micro` instance.
2. **Configure Security Group**: Add an **Inbound Rule** to allow Custom TCP traffic on port `8501` (from anywhere, or your IP).
3. **Connect via SSH**:
   ```bash
...
6. **Execute Application**:
   ...
   ```
7. **Access Application**: Go to `http://<EC2-Public-IP>:8501` on your browser.
